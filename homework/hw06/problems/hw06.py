...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, p):
        if len(incorrect_passwords) >= 3:
            return 'Your account is locked. Attempts: {}'.format(str(incorrect_passwords))
        elif p != password:
            incorrect_passwords.append(p)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def new_withdraw(amount, p):
        if p != new_password and p != old_password:
